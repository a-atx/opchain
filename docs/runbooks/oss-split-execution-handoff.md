...
4. Ship: from a checkout of `origin/main` (detached is fine): `npm run deploy` → `npm run smoke:prod` → verify prod `/api/health` version equals `origin/main` short SHA, `/LICENSE` returns 200 with the Apache-2.0 text (don't assert the content-type — the ASSETS binding may serve extensionless files as octet-stream), `security.txt` is GitHub-first. Close any open deploy-lag issue.
...
**C0 — Prereqs.** `brew install git-filter-repo gitleaks`. Close stale drafts #385/#363 if still open. **Harden `publish-mcp-registry.yml` in a monorepo PR before extraction** — it downloads `mcp-publisher` from `releases/latest` unpinned and runs it with `id-token: write`, and the workflow moves into the product repo at the flip, so the rewritten history must already carry the fix: pin an exact `mcp-publisher` version, verify its `sha256` before executing, and SHA-pin the workflow's `uses:` lines while in there (plan §4.1 rows 14/18). `⛔ HUMAN` — Aidan announces a freeze on `skills/`, `plugins/`, `.claude-plugin/`, `mcp/`, `server.json`, `src/lib/mcp/` (freeze holds until the consume PR merges).

**C1 — Manifest.** Create `split/product-paths.txt` in the monorepo from plan §2.2, then re-derive against the current tree (the seam PRs added `scripts/{check-skill-flags,lint-internal-refs,sync-plugin-skills}.mjs` — decide each per the §2.2 rule: *validators + product runtime → product; site-page derivations → site*; `check-skill-flags` imports the site flag registry and **stays**; `sync-plugin-skills` and `lint-internal-refs` are product-pure by design and **move**). PR it for `⛔ HUMAN` review — the filter run is one-shot.
...
**C3 — Verify the rewrite** (all must hold before anything is pushed): commit count is a sanity check, not a gate — the pre-seam estimate was ≈76–90 and the seam/governance PRs (#440/#445/#447/#448 + the relicense) each touched product paths, so expect ≈90–110; investigate only if wildly outside that; `git tag -l` → `v1.8.0 v1.8.1 v1.8.2`; `plugins/opchain/skills` is a **real directory** (the symlink was materialised by seam S5 — if you see mode `120000`, stop and re-check); `git log --follow -- skills/oc-app-architect/SKILL.md` reaches 2026-04-14; `README.md`/`CONTRIBUTING.md`/`.github/ISSUE_TEMPLATE`/`LICENSE`/`NOTICE` at root; `gitleaks git -v .` clean.

**C4 — Bootstrap commit** (one commit, `-s`, human author) per plan §2.4 step 6: product `package.json` (`name: opchain-skills, version: 1.8.2, license: Apache-2.0, engines: {node: ">=22"}, dependencies: js-yaml` — **not gray-matter**: #443 replaced it with the in-repo `scripts/lib/frontmatter.mjs`, whose actual dependency — shared with `pm-mcp-checks` and the pack validators — is `js-yaml`; `vitest` as the sole devDependency), CI with job ids `validate` / `test` / `hooks` / `plugin-layout` (these exact names — the ruleset requires them), `dco.yml`, rewrite README/CONTRIBUTING (kill "force-push snapshot / private upstream / PRs can't merge"; the governance files extracted from the monorepo root carry over), regenerate the 18-row skill table + issue-template dropdowns from the 29 skill dirs, CHANGELOG note "≤1.8.2 MIT; ≥1.8.3 Apache-2.0". Then locally: `npm test`, `claude plugin validate ./plugins/opchain` if available (else validate `marketplace.json`/`plugin.json` shape by hand), and a local-marketplace install test if a Claude Code CLI is present.
...
**C6 — Consume PR** in the monorepo, per plan §2.5 verbatim: remove product paths, `git submodule add` `opchain-skills` pinned to the bootstrap SHA, top-level `skills` symlink, rewire `package.json` scripts through `OPCHAIN_*` env seams (already landed — mostly path changes), `src/index.js` MCP import path, `actions/checkout` gets `submodules: true` everywhere, npm `prepare` runs submodule init, `deploy.mjs` gains the submodule-drift refusal **and extends its "HEAD reachable from origin/main" refusal to production deploys** (staging-only today; prod running off-main `c14936d` for two weeks was exactly this failure — audit AF-4), Dependabot `gitsubmodule` entry, repoint contribution links in `site/src/lib/repo.ts` (issues/advisories for the product → `opchain-skills` — one file, that was the point), verify `security.txt` still carries both `Acknowledgments` lines (already shipped in #440/#442 — verify, don't re-add), CLAUDE.md mirror section → "Consuming opchain-skills". CI green → `⛔ HUMAN` review → squash-merge.
...
Also in the product half of this release: **add the SHA-pinned enterprise install entry** to `marketplace.json` — a second plugin entry with a `git-subdir` source (`url` + `path: plugins/opchain` + `ref: v1.8.3` + `sha: <tag commit>`) or an `archive` source with `sha256` of the release zip. Seam S5 materialised `plugins/opchain/skills` precisely so this entry can exist (a sparse `git-subdir` clone has no `../../skills`); without it, `strictKnownMarketplaces`-style enterprise policies still can't pin opchain. The default relative-path entry stays for everyone else. `release.yml` needs `permissions: id-token: write, attestations: write` for the provenance attestation.
